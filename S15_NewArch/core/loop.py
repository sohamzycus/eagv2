...
# S20 Fixes: Bootstrap Graph, Final Turn Warning, Globals Injection
...
class AgentLoop4:
    ...
    def _create_bootstrap_context(self, query: str, file_manifest: dict) -> ExecutionContextManager:
        """
        FIX #3: Bootstrap Graph - Create immediate context with "Planning" node.
        
        This prevents the "Infinite Spinner" bug where the UI appears frozen
        during the 10-30 second planning phase.
        """
        bootstrap_graph = {
            "nodes": [
                {
        ...
        # Mark planning as running so UI shows activity
        context.mark_running("PLANNING")
        log_step("🚀 Bootstrap context created - Planning phase visible to UI", symbol="🎯")
        
        return context

    async def run(self, query, file_manifest, globals_schema, uploaded_files):
        # FIX #3: Create Bootstrap Context immediately for UI responsiveness
        bootstrap_context = self._create_bootstrap_context(query, file_manifest)
        console = Console()
        
        # Show initial planning state to user
        log_step("📋 Phase 1: Planning started...", symbol="🔄")
        
        # Phase 1: File Profiling (if files exist)
        file_profiles = {}
        if uploaded_files:
            log_step("📁 Profiling uploaded files...", symbol="📂")
        ...
        log_step("✅ Planning completed!", payload={"nodes": len(plan_graph.get('nodes', []))}, symbol="📊")

        try:
            # Phase 3: 100% NetworkX Graph-First Execution
            context = ExecutionContextManager(
                plan_graph,
                session_id=bootstrap_context.plan_graph.graph['session_id'],  # Preserve session ID
                original_query=query,
                file_manifest=file_manifest
            )
    ...
    async def _execute_step(self, step_id, context):
        ...
        for turn in range(1, max_turns + 1):
            ...
            # FIX #4: Final Turn Warning - Inject warning before last turn
            if turn == max_turns - 1:
                log_step(f"⚠️ FINAL TURN WARNING: {agent_type} approaching max iterations", symbol="🚨")
            ...
            if output.get("call_tool"):
                ...
                try:
                    ...
                    # FIX #4: Enhanced Final Turn Warning
                    if turn >= max_turns - 2:
                        instruction += f"""

🛑 CRITICAL WARNING: You are on turn {turn + 1}/{max_turns}. 
⚠️ STOP BROWSING. STOP SEARCHING. 
✅ You MUST provide your FINAL OUTPUT now using only the data you have.
✅ Summarize what you have found and produce the required output.
❌ Do NOT call any more tools. Any tool call will be ignored.
❌ Do NOT say you need more information - work with what you have."""

                    current_input = build_agent_input(
                        instruction=instruction,
                        previous_output=output,
                        iteration_context={"tool_result": result_str, "turns_remaining": max_turns - turn}
                    )
